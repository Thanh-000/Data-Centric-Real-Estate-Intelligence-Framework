...
DC-REIF should be evaluated as a model-assisted triage system, not as a standalone appraisal engine. The business value comes from organizing review effort: the system converts a full portfolio into a prioritized review queue, explains why a transaction is unusual, and attaches trust signals so an analyst can decide whether further human review is warranted.
...
The map interaction is intentionally separated into focused and contextual modes. A reviewer can inspect only model-flagged cases, then switch to all sales for neighborhood context. Colors encode review outcome and point size follows the absolute anomaly score, so the interface supports triage without implying a final appraisal decision.

The current dashboard remains a prototype. It does not store reviewer decisions, create audit logs, or retrain models. For the final project scope, it satisfies the visualization requirement by turning pipeline artifacts into an interactive review workflow rather than leaving reviewers with disconnected tables and static plots.
...
\begin{enumerate}
    ...
    \item \textbf{Spatial context matters, but the scope remains review triage.} Zipcode segmentation, distance to Seattle core, and prior local market prices support the model, but the output remains a case-review queue.
...
\begin{enumerate}
    ...
    \item \textbf{Track reviewer outcomes.} The next version should capture whether reviewers accept, reject, or defer each model flag; this will enable threshold tuning and false-positive analysis.
    ...
    \item \textbf{Keep SHAP as explanatory context only.} SHAP should remain in the notebook and report because it supports transparency, but its outputs should be described as model behavior explanations rather than causal claims.
\end{enumerate}
...
The dashboard should be treated as a prototype review surface. It demonstrates that the model outputs can be used by stakeholders, but it does not yet provide the controls required for a production review environment. A short, review-focused improvement checklist is provided in Appendix C.
...
Coverage-sharpness tradeoff & Conservative intervals improve coverage but can reduce anomaly sensitivity & Track interval width by price band and recalibrate only after reviewer validation.\\
...
Dashboard prototype & No saved decisions, authentication, audit log, or production access control & Add reviewer decision storage and basic audit logging if the workflow is piloted.\\
...
\section{Appendix C: Supplemental Review Checks}
The main report keeps the review workflow focused. This appendix lists additional metrics and implementation checks that are useful for future iterations but are not required to interpret the current model results.
...
\begin{table}[H]
\centering
\caption{Prototype dashboard improvement checklist}
\label{tab:dashboard_checklist}
...
Export queue & Allows analysts to export filtered cases for review documentation & Medium\\
