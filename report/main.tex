...
\section{Product Operating Model and Business Value}

\subsection{Decision-Support Positioning}
DC-REIF should be evaluated as a data analytics and data science decision-support product, not as a standalone automated appraisal model. The product value comes from organizing review effort: the system converts a full portfolio into a prioritized review queue, explains why a transaction is unusual, and attaches trust signals so an analyst can decide whether the model output deserves action.

The product separates three layers that are often mixed in machine-learning reports. \textbf{Model outputs} are predictions and intervals generated by the valuation engine. \textbf{Trust metrics} describe whether those outputs are empirically reliable under the evaluation protocol. \textbf{Business operation metrics} describe whether the workflow saves analyst time and produces useful review decisions. Table \ref{tab:metric_layers} makes this separation explicit.

\begin{table}[H]
\centering
\caption{Metric layers for a valuation review product}
...
Business operation metric & Flag rate, review queue size, analyst capacity, precision@K, reviewer acceptance rate, time saved, risk exposure reviewed & Does the product improve the valuation review workflow?\\
\bottomrule
\end{tabular}
\end{table}

\subsection{Business KPI Framework}
The current run already supports several operating KPIs, while others require a human-review pilot before they can be measured honestly. Table \ref{tab:business_kpis} distinguishes measured values from next-step product metrics. This avoids overstating model accuracy as business value.

\begin{table}[H]
\centering
\caption{Business KPI framework for valuation review}
\label{tab:business_kpis}
\begin{tabular}{p{0.23\linewidth}p{0.24\linewidth}p{0.20\linewidth}p{0.23\linewidth}}
\toprule
\textbf{KPI} & \textbf{Definition} & \textbf{Current status} & \textbf{Use in operation}\\
\midrule
...
First-pass triage reduction & Records not initially sent to the review queue divided by scored records & 97.09\% & Estimates workload reduction before analyst validation.\\
Review capacity & Number of cases an analyst can review per day or week & Needs pilot input & Converts queue size into staffing and turnaround planning.\\
Time saved & Baseline manual-screening time minus model-assisted review time & Needs pilot input & Measures whether the dashboard creates operational value.\\
Precision@50 / Precision@100 & Share of top ranked flags accepted as valid by reviewers & Needs pilot labels & Tests whether the prioritized queue is useful at practical review depths.\\
Reviewer acceptance rate & Share of model flags marked valid by analysts & Needs pilot labels & Tracks trust and practical usefulness of model signals.\\
Business risk exposure & Dollar amount or share of flagged transaction value reviewed & Needs policy definition & Prioritizes high-impact cases when analyst capacity is limited.\\
\bottomrule
\end{tabular}
\end{table}
...
The current system includes a Streamlit review dashboard that acts as the stakeholder-facing consumption layer. Its purpose is not to replace the statistical report or the reproducible pipeline. Instead, it translates the technical outputs into a review cockpit for a non-technical valuation analyst. The dashboard reads the generated property intelligence table, model-flagged case list, slice summaries, and trust diagnostics, then exposes them through map, table, and summary views.
...
The current dashboard remains a prototype. It does not store reviewer decisions, require authentication, create audit logs, or retrain models. Those features would be required for a production appraisal-support tool. For the final project scope, however, it satisfies the professional visualization requirement by turning pipeline artifacts into an interactive review workflow rather than leaving stakeholders with disconnected tables and static plots.
...
The feedback loop should update both analytics and product design. If false positives concentrate in specific zipcodes, thresholds or interval calibration can be adjusted by segment. If uncertain labels are high because analysts lack context, the dashboard should add comparable-sale links, property detail pages, or reviewer notes. If precision@50 is high but precision@100 drops sharply, the product should expose a smaller default queue and reserve lower-ranked flags for secondary review.
...
\begin{enumerate}
    ...
    \item \textbf{The largest missing business metric is reviewer validation.} The current system can create a queue, but it still needs precision@K, reviewer acceptance rate, time-per-case, and false-positive reason codes from a pilot.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{Measure operational value.} Estimate analyst review capacity, time saved, queue completion time, and business risk exposure reviewed.
...
The reproducibility evidence is the rerunnable pipeline, generated model outputs, report figures, notebook audit trail, and automated tests. Development utility checks may still exist in the repository, but they are not treated as a core analytical deliverable in this final report.
...
The dashboard \path{app/streamlit_app.py} is the product-facing deliverable. It operationalizes the report by answering the question a stakeholder naturally asks after reading the metrics: which properties should be reviewed, where are they located, and how much should the analyst trust the model signal?
...
As a product, the dashboard should be treated as an MVP consumption layer. It demonstrates that the model outputs can be used by stakeholders, but it does not yet provide the full operating controls required in a production review environment. Table \ref{tab:dashboard_roadmap} summarizes the recommended productization roadmap.

\begin{table}[H]
\centering
\caption{Dashboard productization roadmap}
\label{tab:dashboard_roadmap}
\begin{tabular}{p{0.25\linewidth}p{0.36\linewidth}p{0.29\linewidth}}
\toprule
\textbf{Capability} & \textbf{Product value} & \textbf{Priority}\\
\midrule
...
Audit log & Records who reviewed a case, when, and what changed & High for regulated use\\
Export queue & Allows analysts to export filtered cases for committee review or documentation & Medium\\
Confidence filters & Lets users prioritize strong-evidence flags or isolate high-risk low-support slices & Medium\\
...
Authentication and roles & Separates analyst, reviewer, and administrator permissions & Required for production\\
\bottomrule
\end{tabular}
\end{table}

\section{Monitoring, Retraining, and Risk Controls}

\subsection{Monitoring and Retraining Plan}
If DC-REIF is used with new transactions, the main product risk becomes drift. Real estate markets change through seasonality, interest rates, inventory, neighborhood development, and buyer behavior. A model trained on static historical data can remain technically runnable while becoming operationally stale. The system therefore needs monitoring across data, model, uncertainty, and business workflow layers.

\begin{table}[H]
\centering
\caption{Monitoring plan for repeated operational use}
...
A practical retraining cadence should combine schedule-based and trigger-based rules. A scheduled quarterly retraining cycle is reasonable for a prototype operating on new housing data. Trigger-based retraining should occur sooner when price distribution drift, segment support deterioration, MAPE drift, or coverage drift exceeds the organization's tolerance. The same chronological split principle should be retained so that validation continues to reflect future-facing deployment rather than random historical mixing.

\subsection{Risk-Control Table}
The current system is credible as a course-level DA/DS decision-support product, but it is not production-ready in a regulatory appraisal sense. Table \ref{tab:risk_controls} reframes limitations as product risks with explicit controls.

\begin{table}[H]
\centering
\caption{Risk controls for responsible use}
...
The project should be presented as a model-assisted real estate intelligence framework. Its strongest contribution is not only predictive accuracy, but the integration of data governance, leakage-aware modeling, empirical uncertainty diagnostics, SHAP explainability, business KPI framing, and a stakeholder-facing dashboard review workflow. Its main remaining improvement is to collect human review outcomes and current-market data so that precision@K, reviewer acceptance, time saved, and drift controls can be measured in operation.
